---
name: cria-prd
description: Generates a Product Requirements Document (PRD) from a feature prompt. Use when planning a new feature, starting a project, or when asked to create a PRD. Don't use for technical specifications, task breakdowns, or implementation.
---

# Criação de PRD

## Step 0: Verify Dependencies

1. Ask the user which feature to create the PRD for. Identify if a feature prompt exists in `docs/` (e.g., `docs/nome-da-feature.md`).
2. If no prompt file is found, warn the user:
   "[AVISO] Nenhum prompt de feature encontrado em docs/. O prompt ajuda a definir requisitos de forma estruturada."
3. Ask the user: "Deseja continuar sem prompt de referência? Você pode ditar os requisitos diretamente." (use the ask user question tool).
4. If the user chooses to abort, suggest: "Crie um prompt usando o template em `.claude/skills/cria-prd/assets/prompt-template.md` e salve em `docs/`."
5. If the user chooses to continue, proceed to Step 1.

## Step 1: Gather Context

1. If a feature prompt exists, read it fully.
2. Read `CLAUDE.md` to understand the project stack and conventions.
3. Explore the current codebase structure to understand existing components and patterns.

## Step 2: Clarification Questions

1. Based on the prompt and codebase context, identify ambiguities or missing information.
2. Ask the user clarification questions using the ask user question tool. Focus on:
   - Business goals and success metrics
   - User personas and primary use cases
   - Scope boundaries (what is explicitly out of scope)
   - Integration constraints
3. Do NOT proceed to PRD generation without clarification answers.

## Step 3: Generate PRD

1. Read the PRD template at `assets/prd-template.md`.
2. Generate the PRD following the template structure:
   - **Visão Geral**: High-level problem and value proposition
   - **Objetivos**: Measurable goals and KPIs
   - **Histórias de Usuário**: User stories with personas
   - **Funcionalidades Principais**: Feature list with requirements
   - **Experiência do Usuário**: UX flows and accessibility
   - **Restrições Técnicas de Alto Nível**: Constraints only (no implementation details)
   - **Fora de Escopo**: Explicit exclusions
3. Do NOT include implementation details — those belong in the Tech Spec.

## Step 4: Output

1. Create the feature directory: `tasks/prd-[nome-da-feature]/`.
2. Write the PRD to `tasks/prd-[nome-da-feature]/prd.md`.
3. Confirm completion to the user and suggest the next step: "Execute `cria-techspec` para criar a especificação técnica."

## Error Handling

- If the user provides contradictory requirements, list the conflicts and ask for resolution before generating.
- If the feature scope is too large, suggest splitting into multiple PRDs.
